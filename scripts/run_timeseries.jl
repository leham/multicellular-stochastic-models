# Generate time series of the mean protein numbers for the positive feedback loop.
...
function get_mean_timeseries_η_vs_σ_b(n_snapshots, niter_per_snapshot, η_values, σ_b_values, ntrajectories; ntasks=nthreads())
    lk = ReentrantLock()
...
niter_per_snapshot = 500
ntrajectories = 2
niters = n_snapshots * niter_per_snapshot
println("Simulation time = ", n_iters * tau)

# NOTE: 
# η_values = σ_b_values = 0:0.02:1.0 # used for σ_u = 10 case
# ntrajectories = 10 # used for the case of 10 × 10 grid size
...
println("Simulation over $(length(η_values)) × $(length(σ_b_values)) matrix")
println("Run $ntrajectories trajectories up to $niters iterations")
println("----------------------------------------------------------------------")
flush(stdout)

all_means = @time get_mean_timeseries_η_vs_σ_b(n_snapshots, niter_per_snapshot, η_values, σ_b_values, ntrajectories)
@save datapath*"mean_timeseries_eta_vs_sigma_b_sigma_u_$(σ_u[1])_PFL.jld2" η_values σ_b_values all_means